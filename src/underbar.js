(function() {
  ...
  _.last = function(array, n) {
    //if n is larger than the array, return the array:
    if(n != undefined && n > array.length) { return array; }
  ...
  _.each = function(collection, iterator) {
    //check if collection is an array; if so, only iterate over numbered keys
    //(i.e. don't iterate over array properties)
    if(Array.isArray(collection)) {
      ...
      //for objects, iterate over all elements:
  ...
  _.filter = function(collection, test) {
    var filtered = []; //array to store values that pass the test

    _.each(collection, function(elem) {
      if (test(elem)) { filtered.push(elem); }
    });
  ...
  _.every = function(collection, iterator) {
    ...
    return _.reduce(collection, function(allMatch, elem) {
      if (!allMatch) {
        return false;
      } return !!iterator(elem); //THANK YOU BASED GODS FOR THE DOUBLE-NOT OPERATOR
    }, true);
  };
  ...
  _.extend = function(obj) {
    for(var i = 1; i < arguments.length; i++) {
      if(arguments[i] != {}) { //checks that it isn't empty
        var nextObj = arguments[i]
        for(var j in nextObj) {
          obj[j] = nextObj[j];
        }
      }
    }
  ...
  _.memoize = function(func) {
    var results = {}; //object to store previous args

    return function(arg) {
      if(results[arg.toString()] != undefined) {
        return results[arg.toString()];
      } else { 
          results[arg.toString()] = func.apply(this, arguments);
          return results[arg.toString()];
      }
    };
  };
  ...
  _.delay = function(func, wait) {
    ...
    var trueArgs = args.slice(2, args.length);

    return setTimeout(function() {
      return func.apply(null, trueArgs);
    }, wait);
  };
  ...
  _.shuffle = function(array) {
    var shuffle = [];
    var position = 0;
    var index; //a randomly generated index

    while(position < array.length) {
      index = Math.trunc((Math.random()*array.length));
      if(shuffle[index] === undefined) {
        shuffle[index] = array[position];
        position++;
      }
    }
  ...
  _.zip = function() {
    var args = Array.prototype.slice.call(arguments);
    var arrayNum = args.length; //how many arrays were passed into the function?
    var results = []; //array to store the zipped arrays

    //find the longest array in the arguments
    var maxLength = 0;
  ...
  _.difference = function(array) {
    var args = Array.prototype.slice.call(arguments);
    args = args.slice(1, args.length); //arrays to compare array to

    var diff = [];

    _.each(array, function(elem) {
      var compare = elem;
      var different = true; //assume it is different until proven otherwise
